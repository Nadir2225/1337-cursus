...
def plant_factory(platns: list[dict[str, int | str]]) -> list[Plant]:
    ...
    i = 0
    while i < len(plants):
        plants_list.append(Plant(plants[i]["name"], plants[i]["starting_height"], plants[i]["starting_age"]))
        print(f'Created: {plants[i]["name"]} ({plants[i]["starting_height"]}cm, {plants[i]["starting_age"]} days)')
        i += 1
