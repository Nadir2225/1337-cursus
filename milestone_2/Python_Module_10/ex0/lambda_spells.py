...
if __name__ == '__main__':
    artifacts = [
        ...
        {'name': 'Crystal Orb', 'power': 106, 'type': 'weapon'},
        {'name': 'Fire Staff', 'power': 74, 'type': 'relic'},
        {'name': 'Earth Shield', 'power': 72, 'type': 'weapon'}
    ]
    mages = [
        {'name': 'Sage', 'power': 76, 'element': 'water'},
        {'name': 'Nova', 'power': 54, 'element': 'ice'},
        {'name': 'Zara', 'power': 89, 'element': 'lightning'},
        {'name': 'Nova', 'power': 57, 'element': 'wind'},
        {'name': 'Storm', 'power': 68, 'element': 'wind'}
    ]
    spells = ['tsunami', 'darkness', 'earthquake', 'lightning']

    first_artifact = artifact_sorter(artifacts)[0]
    second_artifact = artifact_sorter(artifacts)[1]

    print("\nTesting artifact sorter...")
    print(
        f"{first_artifact['name']} ({first_artifact['power']} "
        f"power) comes before {second_artifact['name']} "
